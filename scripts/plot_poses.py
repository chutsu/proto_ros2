...
def associate(first_list, second_list, t_offset=0.0, max_difference=0.02):
  """
...
if __name__ == "__main__":
  ...
  # gnd_file = args.data_files[0]
  # est_file = args.data_files[1]

  # gnd_data = np.genfromtxt(gnd_file, delimiter=",", skip_header=1)
  # est_data = np.genfromtxt(est_file, delimiter=",", skip_header=1)
  # # eval_traj(gnd_file, est_file)
  # align(gnd_data[:, 1:4].T, est_data[:, 1:4].T)
  ...
  for i, (label, data) in enumerate(series_data.items()):
    ax.plot(data["rx"], data["ry"], label=label, c=colors[i])
